...
function PortalLogin({onLogin}) {
  ...
  return <div className="center"><section className="panel login portal-login"><img className="brand-logo" src="/logo.jpg" alt="Logo Khoa Ngoại Ung Bướu & Chăm sóc giảm nhẹ" /><div className="brand-title">KHOA NGOẠI UNG BƯỚU & CHĂM SÓC GIẢM NHẸ</div><div className="brand-hospital">BỆNH VIỆN HỮU NGHỊ VIỆT TIỆP</div><div className="brand-slogan">Đức trí tận tâm – Nâng tầm chất lượng</div>
    <h1>Nộp hồ sơ</h1>
...
function AdminApp(){
  ...
  if(!token)return <div className="center"><section className="login panel"><img className="brand-logo" src="/logo.jpg" alt="Logo Khoa Ngoại Ung Bướu & Chăm sóc giảm nhẹ"/><div className="brand-title">KHOA NGOẠI UNG BƯỚU & CHĂM SÓC GIẢM NHẸ</div><div className="brand-hospital">BỆNH VIỆN HỮU NGHỊ VIỆT TIỆP</div><div className="brand-slogan">Đức trí tận tâm – Nâng tầm chất lượng</div><h1>Quản lý hồ sơ</h1><p className="muted">Đăng nhập Admin (quản trị viên)</p>{message&&<div className="notice">{message}</div>}<form onSubmit={doLogin}><input value={login.username} onChange={e=>setLogin({...login,username:e.target.value})} placeholder="Tài khoản"/><input type="password" value={login.password} onChange={e=>setLogin({...login,password:e.target.value})} placeholder="Mật khẩu"/><button className="primary">Đăng nhập</button></form><small className="muted">Môi trường demo: admin / change-me. Khi Deploy (đưa lên Internet), hãy đổi ngay bằng biến môi trường.</small><a className="admin-link" href={PORTAL_PATH}>← Trang nộp hồ sơ</a></section></div>;
  return <div className="app"><header><div className="header-brand"><img className="header-logo" src="/logo.jpg" alt="Logo"/><div><h1>KHOA NGOẠI UNG BƯỚU & CHĂM SÓC GIẢM NHẸ</h1><p>BỆNH VIỆN HỮU NGHỊ VIỆT TIỆP · Admin (quản trị viên)</p></div></div><button className="ghost" onClick={logout}>Đăng xuất</button></header>
